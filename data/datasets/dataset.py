...
class Dataset(object):
    ...
    def download_dataset(self, dataset_dir, dataset_url, name='', gdrive=False):
        ...
        if not name:
            name = osp.basename(dataset_url)

        print('Creating directory "{}"'.format(dataset_dir))
        mkdir_if_missing(dataset_dir)
        fpath = osp.join(dataset_dir, name)

        print('Downloading {} dataset to "{}"'.format(self.__class__.__name__, dataset_dir))
        if gdrive:
            gdown.download(dataset_url, name, quiet=False)
        else:
        ...
        print('Removing zipped file "{}"'.format(fpath))
